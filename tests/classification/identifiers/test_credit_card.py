...
def test_from_dataset():
    ...
    with (Path(__file__).parent / "data" / "credit_card_numbers.json").open() as input:
        for value in json.load(input):
            ret = identifier.debug_values(value)

            if ret == 0:
                valid.append(value)

            elif ret == 1:
                invalid_checksum.append(value)

            elif ret == 2:
                invalid_type.append(value)
    ...
    print()
    print(f"{len(valid)=}")
    print(f"{len(invalid_type)=}")
    print(f"{len(invalid_checksum)=}")
    print(f"{len(others)=}")

    with open("/tmp/invalid-checksum-list.txt", "w") as out:
        for i, c in enumerate(set(invalid_checksum)):
            print(c, file=out)
        print(i)
    with open("/tmp/invalid-type-list.txt", "w") as out:
        for i, c in enumerate(set(invalid_type)):
            print(c, file=out)
        print(i)
